(async () => {
const config = window.ROBIOTEC_WEB_APP_CONFIG || {};
...
const CAN_MANAGE_VEHICLE_REGISTRY = USER_IS_ADMIN
  ...
  || Boolean(config.canManageVehicleRegistry);
...
const HIGH_VALUE_OBJECTIVE_REFRESH_MS = 10000;
...
const VEHICLE_REGISTRY_REFRESH_MS = 15000;
...
const userAdminRefreshButtons = Array.from(document.querySelectorAll("#user-admin-refresh, [data-user-admin-refresh]"));
...
const adminOrganizationFilter = document.getElementById("admin-organization-filter");
...
let vehicleRegistryRefreshInFlight = false;
let fleetGeofenceRefreshInFlight = false;
let highValueObjectivesRefreshInFlight = false;
let lastHighValueObjectivesRefreshAt = 0;
...
let activeAdminOrganizationFilter = "";
...
function getMapMarkerIcon({ powered = true, active = false, markerKind = "camera" } = {}) {
  ...
  if (markerKind === "car") {
    const cacheKey = `car:${powered ? "on" : "off"}:${active ? "active" : "idle"}`;
    const cachedIcon = mapMarkerIconCache.get(cacheKey);
    if (cachedIcon) {
      return cachedIcon;
    }
    ...
    const size = active ? 54 : 46;
    // Verde = activo (recibe telemetría), Rojo = apagado (sin telemetría >2min)
    const filterStyle = powered
      ? "drop-shadow(0 0 5px #22c55e) brightness(1.1)"
      : "sepia(1) saturate(6) hue-rotate(-20deg) drop-shadow(0 0 5px #ef4444) brightness(0.85)";
    const icon = window.L.divIcon({
      className: active ? "ops-map-marker is-active" : "ops-map-marker",
      html: `<img src="${SPY_CAR_ICON_URL}" style="width:${size}px;height:${size}px;filter:${filterStyle};display:block;">`,
      iconSize: [size, size],
      iconAnchor: [Math.round(size / 2), size - 4],
      popupAnchor: [0, -size + 10],
      tooltipAnchor: [0, -size + 10],
...
function refreshRenderedMapMarkers() {
  for (const marker of mapMarkers.values()) {
    ...
    const selectionKey = String(marker.__selectionKey || marker.__deviceId || "");
    updateMapMarkerStyle(marker, {
      powered: isCameraPowered(cameraName),
      active: activeTelemetryDeviceId ? selectionKey === activeTelemetryDeviceId : cameraName === activeCamera,
...
function stopTelemetryMapInitialPreview() {
  telemetryMapInitialPreviewUntil = 0;
  if (telemetryMapInitialPreviewTimerId) {
    clearTimeout(telemetryMapInitialPreviewTimerId);
    telemetryMapInitialPreviewTimerId = null;
...
function centerTelemetryMapOnItem(item, { manual = true, animate = true } = {}) {
  if (!item || !hasValidCoordinates(item) || !ensureMap()) return false;
  const point = [Number(item.lat), Number(item.lon)];
  stopTelemetryMapInitialPreview();
  setTelemetryMapManualControl(manual);
  mapAutoFitDone = true;
  lastTelemetryBoundsSignature = mapCoordinateSignature([point]);
  markTelemetryMapProgrammaticInteraction(TELEMETRY_SELECTION_MAP_MOVE_MS);
  mapInstance.invalidateSize();
  mapInstance.setView(
    point,
    Math.min(TELEMETRY_MAP_MAX_ZOOM, TELEMETRY_SELECTED_VEHICLE_ZOOM),
    { animate },
  );
  return true;
}
...
function requestTelemetryMapRecenter() {
  ...
  if (activeTelemetryDeviceId) {
    ...
    if (centerTelemetryMapOnItem(selectedItem, { manual: true })) {
      updateMap(lastTelemetrySnapshot);
      return;
    }
  }
...
function setupEmbeddedCameraAdminPanel() {
...
function closeCameraRegisterModal() {
  if (!cameraRegisterModal) return;
  if (cameraRegisterModal.classList.contains("is-admin-embedded")) return;
...
function syncVehicleRegisterCatalogOptions({ selectedLinks } = {}) {
  const organizations = filterAdminItemsByOrganization(
    Array.isArray(vehicleRegistryOptionCatalog.organizations)
      ? vehicleRegistryOptionCatalog.organizations
      : [],
  );
  const owners = filterAdminItemsByOrganization(
    Array.isArray(vehicleRegistryOptionCatalog.owners)
      ? vehicleRegistryOptionCatalog.owners
      : [],
  );
  const vehicleTypes = getVehicleTypeCatalog();
  const organizationValues = new Set(organizations.map((organization) => String(organization && organization.id || "").trim()));
  const currentOrganizationValue = String(vehicleRegisterOrganization && vehicleRegisterOrganization.value || "").trim();
  const selectedOrganization = getAdminOrganizationFilterState();
  const preferredOrganizationValue = selectedOrganization.numericId && organizationValues.has(selectedOrganization.numericId)
    ? selectedOrganization.numericId
    : "";
  const nextOrganizationValue = organizationValues.has(currentOrganizationValue)
    ? currentOrganizationValue
    : String(preferredOrganizationValue || (organizations[0] && organizations[0].id) || "");
  const nextOwnerValue = String(
    (vehicleRegisterOwner && owners.some((owner) => String(owner && owner.id || "").trim() === String(vehicleRegisterOwner.value || "").trim()) && vehicleRegisterOwner.value)
...
function renderVehicleRegisterCameraList(selectedLinks = null) {
  ...
  const cameras = filterAdminItemsByOrganization(
    Array.isArray(vehicleRegistryOptionCatalog.cameras)
...
async function refreshVehicleRegistryFormOptions() {
  if (!vehicleRegisterForm || !CAN_MANAGE_VEHICLE_REGISTRY) {
    if (vehicleRegisterOpen) {
      vehicleRegisterOpen.hidden = true;
    }
    return;
  }
  try {
...
function friendlyOrganizationAdminError(error) {
  ...
  switch (code) {
    ...
    case "organization_not_found":
      return "La organización seleccionada ya no existe.";
    case "organization_scope_forbidden":
      return "Solo puedes gestionar organizaciones dentro de tu alcance.";
...
async function registerVehicle(event) {
  ...
  if (!CAN_MANAGE_VEHICLE_REGISTRY) {
    setVehicleRegisterFeedback("Tu rol solo permite consultar kilómetros de la flota.", "error");
    return;
  }
  if (
    !vehicleRegisterType
...
function normalizeAdminOrganizationFilterId(value) {
  const text = String(value || "").trim();
...
function getAdminOrganizationFilterState() {
  if (!adminOrganizationFilter) {
    return { sourceId: "", numericId: "", label: "" };
  }
  const selectedOption = adminOrganizationFilter.options[adminOrganizationFilter.selectedIndex] || null;
  const sourceId = normalizeAdminOrganizationFilterId(
    adminOrganizationFilter.value
    || (selectedOption && selectedOption.dataset.orgSourceId)
    || "",
  );
  const numericId = normalizeAdminOrganizationFilterId(
    selectedOption && selectedOption.dataset.orgNumericId || "",
  );
  return {
    sourceId,
    numericId,
    label: selectedOption ? String(selectedOption.textContent || "").trim() : "",
  };
}

function adminOrganizationFilterIsActive() {
  const filter = getAdminOrganizationFilterState();
  return Boolean(filter.sourceId || filter.numericId);
}

function collectAdminOrganizationIds(item) {
  const source = item && typeof item === "object" ? item : {};
  const looksLikeOrganization = Boolean(
    source.source_id
    && Object.prototype.hasOwnProperty.call(source, "nombre")
    && Object.prototype.hasOwnProperty.call(source, "activa")
    && !Object.prototype.hasOwnProperty.call(source, "tipo_camara_codigo")
    && !Object.prototype.hasOwnProperty.call(source, "entry_kind")
    && !Object.prototype.hasOwnProperty.call(source, "username")
  );
  const sourceCandidates = [
    source.company_id,
    source.company_source_id,
    source.organization_source_id,
    source.organizacion_source_id,
    source.source_company_id,
    source.company_uuid,
    looksLikeOrganization ? source.source_id : "",
  ].map(normalizeAdminOrganizationFilterId).filter(Boolean);
  const numericCandidates = [
    source.company_numeric_id,
    source.company_num_id,
    source.organization_id,
    source.organizacion_id,
    looksLikeOrganization ? source.id : "",
  ].map(normalizeAdminOrganizationFilterId).filter(Boolean);
  return {
    sourceIds: new Set(sourceCandidates),
    numericIds: new Set(numericCandidates),
  };
}

function adminItemMatchesOrganizationFilter(item) {
  const filter = getAdminOrganizationFilterState();
  if (!filter.sourceId && !filter.numericId) return true;
  const ids = collectAdminOrganizationIds(item);
  return Boolean(
    (filter.sourceId && ids.sourceIds.has(filter.sourceId))
    || (filter.numericId && ids.numericIds.has(filter.numericId))
  );
}

function filterAdminItemsByOrganization(items) {
  const source = Array.isArray(items) ? items : [];
  if (!adminOrganizationFilterIsActive()) return [...source];
  return source.filter(adminItemMatchesOrganizationFilter);
}

function populateAdminOrganizationFilter(organizations) {
  if (!adminOrganizationFilter) return;
  const source = Array.isArray(organizations) ? organizations : [];
  const currentValue = String(adminOrganizationFilter.value || activeAdminOrganizationFilter || "").trim();
  const options = [
    '<option value="">Todas las organizaciones</option>',
    ...source.map((item) => {
      const sourceId = String(item && item.source_id || "").trim();
      const numericId = String(item && item.id || "").trim();
      const name = String(item && item.nombre || "").trim();
      if (!sourceId || !name) return "";
      return `<option value="${escapeHtml(sourceId)}" data-org-source-id="${escapeHtml(sourceId)}" data-org-numeric-id="${escapeHtml(numericId)}">${escapeHtml(name)}</option>`;
    }),
  ].filter(Boolean);
  adminOrganizationFilter.innerHTML = options.join("");
  const hasCurrent = Array.from(adminOrganizationFilter.options || []).some((option) => option.value === currentValue);
  adminOrganizationFilter.value = hasCurrent ? currentValue : "";
  activeAdminOrganizationFilter = adminOrganizationFilter.value;
}

function setActiveAdminTab(tabName, { updateHash = false } = {}) {
  if (!adminTabs.length || !adminTabPanels.length) return;
  const availableTabs = adminTabs.filter((button) => !button.hidden);
  const fallback = availableTabs[0] && availableTabs[0].dataset.adminTab || "";
  const requestedTab = String(tabName || fallback || "").trim();
  const nextTab = availableTabs.some((button) => String(button.dataset.adminTab || "") === requestedTab)
    ? requestedTab
    : fallback;
  adminTabs.forEach((button) => {
    const isActive = String(button.dataset.adminTab || "") === nextTab;
    button.classList.toggle("is-active", isActive);
    button.setAttribute("aria-selected", isActive ? "true" : "false");
  });
  adminTabPanels.forEach((panel) => {
    const isActive = String(panel.dataset.adminTabPanel || "") === nextTab;
...
function syncAdminTabAvailability() {
  if (!adminTabs.length || !adminTabPanels.length) return;
  adminTabs.forEach((button) => {
    const tabName = String(button.dataset.adminTab || "").trim();
    const panel = adminTabPanels.find((item) => String(item.dataset.adminTabPanel || "").trim() === tabName);
...
function syncUserAdminFormState({ preserveDraft = true } = {}) {
  ...
  if (!preserveDraft || isEditing) {
    ...
    if (userAdminOrganization) {
      const preferredOrganization = getAdminOrganizationFilterState().sourceId;
      userAdminOrganization.value = isEditing ? String(selectedUser.company_id || "") : preferredOrganization;
    }
...
function resetUserAdminForm({ preserveFeedback = false } = {}) {
  ...
  if (userAdminOrganization) userAdminOrganization.value = getAdminOrganizationFilterState().sourceId || "";
...
function syncOrganizationAdminFormState({ preserveDraft = true } = {}) {
  ...
  if (organizationAdminDetailCopy) {
    organizationAdminDetailCopy.textContent = isEditing
      ? "Actualiza el nombre, la descripcion o el estado de la organizacion seleccionada."
      : "Primero crea la organizacion. Despues crea o edita usuarios y asignales esta organizacion desde Gestion de Accesos.";
  }
...
function syncCameraAdminFormState({ preserveDraft = true } = {}) {
  ...
  if (!preserveDraft || isEditing || isEditingRbox) {
    ...
    if (cameraAdminOrganization) {
      const preferredOrganization = getAdminOrganizationFilterState().numericId;
      cameraAdminOrganization.value = selectedCamera
        ? String(selectedCamera.organizacion_id || "")
        : (cameraAdminOrganization.value || preferredOrganization || "");
    }
    if (cameraAdminOwner) {
...
function resetCameraAdminForm({ preserveFeedback = false, creationMode = "camera" } = {}) {
  ...
  if (cameraAdminOrganization) cameraAdminOrganization.value = getAdminOrganizationFilterState().numericId || "";
...
function renderCameraAdmin(cameras, options, { preserveDraft = true } = {}) {
  lastCameraAdminSnapshot = filterAdminItemsByOrganization(cameras);
  const filteredOrganizations = filterAdminItemsByOrganization(Array.isArray(options && options.organizations) ? options.organizations : []);
  const filteredOwners = filterAdminItemsByOrganization(Array.isArray(options && options.owners) ? options.owners : []);
  const filteredVehicles = filterAdminItemsByOrganization(Array.isArray(options && options.vehicles) ? options.vehicles : []);
  const filteredRboxes = filterAdminItemsByOrganization(Array.isArray(options && options.rboxes) ? options.rboxes : []);
...
function renderUserAdmin(users, roles, { preserveDraft = true, roleOptions = roles, organizations = [] } = {}) {
  lastUserAdminSnapshot = filterAdminItemsByOrganization(users);
  ...
  renderUserAdminOrganizationOptions(filterAdminItemsByOrganization(Array.isArray(organizations) ? organizations : lastOrganizationAdminSnapshot));
...
function renderOrganizationAdmin(organizations, users, { preserveDraft = true } = {}) {
  lastOrganizationAdminSnapshot = filterAdminItemsByOrganization(organizations);
  renderOrganizationAdminOwnerOptions(filterAdminItemsByOrganization(users));
  if (
    selectedOrganizationAdminId !== null
...
async function refreshUserAdmin({ preserveDraft = true } = {}) {
  ...
  try {
    ...
    populateAdminOrganizationFilter(organizations);
...
async function submitOrganizationAdminForm(event) {
  event.preventDefault();
  if (!organizationAdminName || !organizationAdminSubmit) return;
  ...
  try {
    ...
    if (ownerUserId) {
      requestBody.propietario_usuario_id = Number(ownerUserId);
    }
...
function buildSynchronizedTelemetrySnapshot(items) {
  ...
  source.forEach((item) => {
    ...
    merged.set(key, {
      ...item,
      device_id: deviceId,
      vehicle_source_id: sourceId || String(item.vehicle_source_id || "").trim(),
  ...
  DEVICES.forEach((device) => {
    ...
    if (isMountedCameraDevice(device)) {
      ...
      if (mountedTelemetry) {
        ...
        const mountedKey = telemetrySelectionKey(mountedTelemetry) || mountedDeviceId;
        if (mountedKey) {
          const mountedSourceId = telemetryVehicleSourceId(mountedTelemetry) || telemetryVehicleSourceId(device);
          merged.set(mountedKey, {
            ...mountedTelemetry,
            ...
            vehicle_source_id: mountedSourceId || String(mountedTelemetry.vehicle_source_id || "").trim(),
    ...
    const deviceKey = telemetrySelectionKey(device) || deviceId;
    const deviceSourceId = telemetryVehicleSourceId(device);
    const existing = merged.get(deviceKey);
    const nextItem = existing
      ? {
          ...existing,
          device_id: deviceId,
          vehicle_source_id: telemetryVehicleSourceId(existing) || deviceSourceId || String(existing.vehicle_source_id || "").trim(),
      ...
      : {
          device_id: deviceId,
          vehicle_source_id: deviceSourceId,
...
async function refreshHighValueObjectives() {
  if (!mapInstance || typeof window.L === "undefined") return;
  if (highValueObjectivesRefreshInFlight) return;
  const now = Date.now();
  if (now - lastHighValueObjectivesRefreshAt < HIGH_VALUE_OBJECTIVE_REFRESH_MS) return;
  highValueObjectivesRefreshInFlight = true;
  lastHighValueObjectivesRefreshAt = now;
  ...
  try {
    ...
    highValueObjectivesRefreshInFlight = false;
  }
}
...
function focusLocation(cameraName) {
  ...
  const telemetryMarker = device
    ? mapMarkers.get(telemetryMarkerKey(device)) || Array.from(mapMarkers.values()).find((marker) => String(marker.__cameraName || "") === cameraName)
...
function renderManualVehicleRegistryItem(item) {
  ...
  return `
    ...
    <!--
      ...
      ${item.registration_id ? renderVehicleKmPanel(item) : ""}
    </article>
  `;
}

// Estado persistente del panel km por vehículo (sobrevive re-renders del polling)
const _kmPanelState = new Map();

function _ecuadorToday() {
...
function _restoreKmPanelState(registrationId) {
  ...
  if (!panel) return;
  const fromEl = panel.querySelector(".vehicle-km-from");
  const toEl = panel.querySelector(".vehicle-km-to");
  ...
  if (saved.resultsHtml) {
    ...
    if (resultsEl) resultsEl.innerHTML = saved.resultsHtml;
  }
  if (saved.exportHref) {
...
function renderVehicleKmPanel(item) {
  const rid = String(item.registration_id);
  const saved = _kmPanelState.get(rid);
  const ecOffsetMs = 5 * 3600 * 1000;
  const today = new Date(Date.now() - ecOffsetMs).toISOString().slice(0, 10);
  const from30 = new Date(Date.now() - ecOffsetMs - 29 * 86400000).toISOString().slice(0, 10);
  const fromVal = saved?.from || from30;
  const toVal = saved?.to || today;
  return `
    <div class="vehicle-km-panel"
         data-vehicle-id="${escapeHtml(rid)}"
         data-vehicle-label="${escapeHtml(String(item.label || item.identifier || "vehiculo"))}">
      <div class="vehicle-km-panel-title">Recorridos y Kilómetros</div>
      <div class="vehicle-km-controls">
        <label class="vehicle-km-label">Desde
          <input type="date" class="vehicle-km-from" value="${fromVal}" max="${today}">
        </label>
        <label class="vehicle-km-label">Hasta
          <input type="date" class="vehicle-km-to" value="${toVal}" max="${today}">
        </label>
        <button class="vehicle-km-load" type="button">Cargar</button>
        <a class="vehicle-km-export" href="#" style="display:none">Exportar CSV</a>
      </div>
      <div class="vehicle-km-results"></div>
    </div>
  `;
}
...
function renderVehicleRegistry(items) {
  ...
  const source = filterAdminItemsByOrganization(items);
...
async function refreshVehicleRegistry() {
  if (!vehicleRegistryDetail && !vehicleRegistryRailList) return;
  if (document.visibilityState === "hidden") return;
  if (vehicleRegistryRefreshInFlight) return;
  vehicleRegistryRefreshInFlight = true;
  try {
    ...
    vehicleRegistryRefreshInFlight = false;
  }
}
...
function vehicleFreshnessLabel(freshness) {
  const f = (freshness || "").toLowerCase();
  if (f === "fresh" || f === "stale") return "ACTIVO";
  if (f === "lost") return "APAGADO";
  return "SIN SEÑAL";
}

function isVehicleOperational(item) {
  const extra = item && typeof item.extra === "object" && item.extra ? item.extra : {};
  if (item && item.operational === false) return false;
  if (extra.operational === false) return false;
  const status = String(item?.operational_status || extra.operational_status || extra.status || "").trim().toUpperCase();
  if (status.includes("NO OPERATIVO") || status.includes("INOPERATIVO") || status.includes("FUERA DE SERVICIO")) {
    return false;
  }
  if (status.includes("OPERATIVO")) {
    return String(item?.freshness || "").toLowerCase() !== "lost";
  }
  return String(item?.freshness || "").toLowerCase() !== "lost";
}

function vehicleOperationalLabel(item) {
  const extra = item && typeof item.extra === "object" && item.extra ? item.extra : {};
  const status = String(item?.operational_status || extra.operational_status || "").trim();
  if (status) return status.toUpperCase();
  return vehicleFreshnessLabel(item?.freshness);
}
...
function telemetryExtra(item) {
  return item && item.extra && typeof item.extra === "object" ? item.extra : {};
}

function telemetryVehicleSourceId(item) {
  if (!item || typeof item !== "object") return "";
...
function telemetryDeviceIdentifier(item) {
  if (!item || typeof item !== "object") return "";
  const extra = telemetryExtra(item);
  return String(
    item.device_id
...
function telemetrySelectionKey(item) {
  return telemetryVehicleSourceId(item) || telemetryDeviceIdentifier(item);
}

function telemetryMarkerKey(item) {
  return telemetrySelectionKey(item) || telemetryLabel(item);
}
...
function telemetryMatchesSelection(item, selectionKey) {
  ...
  if (deviceId && deviceId === key) return true;
  return telemetryMarkerKey(item) === key;
}
...
function isSelectedTelemetryItem(item) {
  if (!item || typeof item !== "object") return false;
  if (activeTelemetryDeviceId) {
    return telemetryMatchesSelection(item, activeTelemetryDeviceId);
  }
...
function getSelectableTelemetryItems(items) {
  return (Array.isArray(items) ? items : [])
    .filter((item) => {
      ...
      return Boolean(telemetrySelectionKey(item));
    })
    .sort((left, right) => telemetryLabel(left).localeCompare(telemetryLabel(right)));
}
...
function getSelectedTelemetryItem(items) {
  ...
  return source.find((item) => telemetryMatchesSelection(item, activeTelemetryDeviceId)) || null;
}
...
function setTelemetrySelection(deviceId, { recenter = true } = {}) {
  ...
  if (recenter) {
    if (activeTelemetryDeviceId) {
      const selectedItem = getSelectedTelemetryItem(lastTelemetrySnapshot);
      centerTelemetryMapOnItem(selectedItem, { manual: true });
      updateMap(lastTelemetrySnapshot);
      return;
    }
...
function telemetryHighlights(item) {
  ...
  if (extra.source === "artemis") {
    if (extra.status)  parts.push(`Estado: ${String(extra.status)}`);
    if (extra.company) parts.push(`Empresa: ${String(extra.company)}`);
    if (extra.event)   parts.push(`Evento: ${String(extra.event)}`);
    if (extra.zone)    parts.push(`Zona: ${String(extra.zone)}`);
    if (extra.address) parts.push(`Dirección: ${String(extra.address)}`);
    return parts.join(" · ");
  }
...
function miningConcessionForItem(item) {
  ...
  const deviceId = telemetrySelectionKey(item);
...
function ensureFleetGeofenceLayer() {
  ...
  return fleetGeofenceLayer;
}

function normalizeGeofenceColor(value, fallback = GEOFENCE_DEFAULT_COLOR) {
  const raw = String(value || "").trim();
  if (/^#[0-9a-f]{3}([0-9a-f]{3})?$/i.test(raw)) return raw.toLowerCase();
  return fallback;
}

function geofenceColor(item, fallback = GEOFENCE_DEFAULT_COLOR) {
  const geometry = item && typeof item.geometry === "object" && item.geometry ? item.geometry : {};
  const style = geometry.style && typeof geometry.style === "object" ? geometry.style : {};
  return normalizeGeofenceColor(item?.color || geometry.color || style.color || style.fillColor, fallback);
}

function geofenceCompanyIdFromItem(item) {
  if (!item || typeof item !== "object") return "";
  const extra = item.extra && typeof item.extra === "object" ? item.extra : {};
  return String(
    item.company_id
    ...
    || extra.organizacion_id
    || "",
  ).trim();
}

function inferGeofenceCompanyId() {
  const selectedItem = getSelectedTelemetryItem(lastTelemetrySnapshot);
  const selectedCompanyId = geofenceCompanyIdFromItem(selectedItem);
  if (selectedCompanyId) return selectedCompanyId;

  const companyIds = new Set(
    (Array.isArray(lastTelemetrySnapshot) ? lastTelemetrySnapshot : [])
      .map((item) => geofenceCompanyIdFromItem(item))
      .filter(Boolean),
  );
...
function setGeofenceDrawFeedback(message = "", tone = "") {
  if (!geofenceDrawFeedback) return;
  geofenceDrawFeedback.textContent = message;
  geofenceDrawFeedback.dataset.tone = tone;
}

function updateGeofenceDrawControls() {
  const pointCount = geofenceDraftPoints.length;
  const hasName = Boolean(String(geofenceDrawName?.value || "").trim());
  if (geofenceDrawCount) {
    geofenceDrawCount.textContent = `${pointCount} punto${pointCount === 1 ? "" : "s"}`;
  }
  if (geofenceDrawSave) {
    geofenceDrawSave.disabled = pointCount < 3 || !hasName;
  }
}

function ensureGeofenceDraftLayer() {
  if (!mapInstance || typeof window.L === "undefined") return null;
  if (!geofenceDraftLayer) {
    geofenceDraftLayer = window.L.layerGroup().addTo(mapInstance);
  }
  return geofenceDraftLayer;
}

function clearGeofenceDraftLayer() {
  if (geofenceDraftLayer) {
    geofenceDraftLayer.clearLayers();
  }
}

function renderGeofenceDraft() {
  const layer = ensureGeofenceDraftLayer();
  if (!layer || typeof window.L === "undefined") return;
  layer.clearLayers();
  const color = normalizeGeofenceColor(geofenceDrawColor?.value);
  const latlngs = geofenceDraftPoints.map((point) => [point.lat, point.lon]);
  ...
  if (latlngs.length >= 2) {
    window.L.polyline(latlngs, {
      ...
      weight: 3,
      opacity: 0.92,
      dashArray: latlngs.length >= 3 ? null : "6 6",
    }).addTo(layer);
  }
  if (latlngs.length >= 3) {
...
function stopFleetGeofencePolygonDraw({ clearDraft = true } = {}) {
  if (!mapInstance) return;
  mapInstance.off("click", handleGeofenceDraftMapClick);
  mapInstance.off("dblclick", handleGeofenceDraftMapDoubleClick);
  if (geofenceDraftDoubleClickZoomWasEnabled && mapInstance.doubleClickZoom) {
...
function handleGeofenceDraftMapClick(event) {
  if (!geofenceDrawingActive || !event?.latlng) return;
  event.originalEvent?.preventDefault?.();
  event.originalEvent?.stopPropagation?.();
  const lat = Number(event.latlng.lat);
  const lon = Number(event.latlng.lng);
  const lastPoint = geofenceDraftPoints[geofenceDraftPoints.length - 1];
  if (
    lastPoint
    && Math.abs(lastPoint.lat - lat) < 0.0000001
    && Math.abs(lastPoint.lon - lon) < 0.0000001
  ) {
    return;
  }
  geofenceDraftPoints.push({
    lat,
    lon,
  });
  renderGeofenceDraft();
...
function handleGeofenceDraftMapDoubleClick(event) {
  if (!geofenceDrawingActive) return;
  event.originalEvent?.preventDefault?.();
  event.originalEvent?.stopPropagation?.();
  if (geofenceDraftPoints.length >= 3 && String(geofenceDrawName?.value || "").trim()) {
    void saveFleetGeofencePolygon();
  }
}
...
async function saveFleetGeofencePolygon() {
  ...
  const color = normalizeGeofenceColor(geofenceDrawColor?.value);
  const companyId = inferGeofenceCompanyId();
  const payload = {
    name,
    type: "polygon",
    color,
    geometry: {
      ...
      coordinates: geofenceDraftPoints.map((point) => [point.lat, point.lon]),
      style: {
        color,
        fillColor: color,
      },
  ...
  try {
    ...
    setGeofenceDrawFeedback(String(error?.message || "No se pudo guardar la geocerca."), "error");
    updateGeofenceDrawControls();
  }
}

function geofenceCircleData(item) {
  const geometry = item && typeof item.geometry === "object" && item.geometry ? item.geometry : {};
  const center = geometry.center && typeof geometry.center === "object" ? geometry.center : {};
  const lat = Number(center.lat ?? geometry.lat);
  const lon = Number(center.lon ?? center.lng ?? geometry.lon ?? geometry.lng);
  const radius = Number(geometry.radius_m ?? geometry.radius ?? item?.radius_m);
  if (!Number.isFinite(lat) || !Number.isFinite(lon) || !Number.isFinite(radius) || radius <= 0) return null;
  return { lat, lon, radius };
}

function geofencePolygonLatLngs(item) {
  const geometry = item && typeof item.geometry === "object" && item.geometry ? item.geometry : {};
  let coordinates = Array.isArray(geometry.coordinates) ? geometry.coordinates : [];
  if (
    String(geometry.type || "").toLowerCase() === "polygon"
    && String(geometry.coordinate_order || "").toLowerCase() !== "latlon"
  ...
  return coordinates
    .map((point) => {
      ...
      if (point && typeof point === "object") return [Number(point.lat), Number(point.lon ?? point.lng)];
      return null;
    })
    .filter((point) => point && Number.isFinite(point[0]) && Number.isFinite(point[1]));
}
...
function renderFleetGeofences(items) {
...
async function refreshFleetGeofences() {
  if (!mapInstance) return;
  if (fleetGeofenceRefreshInFlight) return;
  fleetGeofenceRefreshInFlight = true;
  try {
    const data = await fetchJson("/api/geofences", { timeoutMs: 8000 });
    lastFleetGeofences = Array.isArray(data) ? data : [];
    renderFleetGeofences(lastFleetGeofences);
  } catch (error) {
    lastFleetGeofences = [];
    renderFleetGeofences([]);
  } finally {
    fleetGeofenceRefreshInFlight = false;
  }
}
...
async function refreshSelectedMiningConcession(item) {
  ...
  const nextDeviceId = telemetrySelectionKey(item);
  ...
  return `
    <div class="telemetry-focus-summary">
      ...
      <div class="telemetry-summary telemetry-summary-embedded">
        ${selectable.map((item) => `
          <article class="telemetry-row ${isSelectedTelemetryItem(item) ? "is-active" : ""} is-registered-drone" data-telemetry-device-id="${escapeHtml(telemetrySelectionKey(item))}" data-selectable="true">
...
function renderTelemetryFocus(items) {
  ...
  const isArtemis = extra.source === "artemis";
  const ignitionLabel = extra.ignition === "on" ? "ENCENDIDO" : extra.ignition === "off" ? "APAGADO" : "--";
  const motionStats = isArtemis ? [
    { label: "Placa",       value: String(extra.plate || item.display_name || "--") },
    { label: "Motor",       value: ignitionLabel },
    { label: "Hora GPS",    value: String(extra.gps_datetime || "--") },
    { label: "Velocidad",   value: formatTelemetryValue(item.speed, " km/h") },
    { label: "Rumbo",       value: formatTelemetryValue(item.heading, "°") },
    { label: "Latitud",     value: formatLocationCoordinate(item.lat ?? null) },
    { label: "Longitud",    value: formatLocationCoordinate(item.lon ?? null) },
    { label: "Estado operativo", value: vehicleOperationalLabel(item) },
    { label: "Estado GPS",  value: vehicleFreshnessLabel(item.freshness) },
    { label: "Empresa",     value: String(extra.company || "--") },
    { label: "Zona",        value: String(extra.zone || "--") },
    { label: "Evento",      value: String(extra.event || "--") },
    { label: "Dirección",   value: String(extra.address || "--") },
  ] : [
  ...
  telemetryFocusCard.innerHTML = `
    <div class="telemetry-focus-content">
      ...
      ${showBatteryCard ? `<div class="telemetry-battery-card" data-level="${batteryTone}">
...
function updateMap(items) {
  ...
  mapInstance.invalidateSize();
  if (typeof syncVehicleRoute === "function") syncVehicleRoute(items);
  ...
  valid.forEach((item) => {
    ...
    const cameraName = String(item.camera_name || "");
    const selectionKey = telemetrySelectionKey(item);
    const markerKey = telemetryMarkerKey(item);
    if (!markerKey) return;
    const kind = telemetryMarkerKind(item);
    // Para vehículos: verde si recibe telemetría (fresh/stale), rojo si apagado (lost)
    const powered = (kind === "car" || kind === "drone")
      ? isVehicleOperational(item)
      : isCameraPowered(cameraName);
    const isActive = isSelectedTelemetryItem(item);
    let marker = mapMarkers.get(markerKey);
    if (!marker) {
      ...
      marker.on("click", () => {
        if (telemetryDeviceFilter && marker.__selectionKey && marker.__isRegisteredVehicle) {
          setTelemetrySelection(marker.__selectionKey, { recenter: true });
    ...
    marker.__selectionKey = selectionKey || markerKey;
    marker.__isRegisteredVehicle = isRegisteredVehicleTelemetry(item);
    marker.__markerKind = telemetryMarkerKind(item);
    const isVehicle = kind === "car" || kind === "drone";
    const statusLabel = isVehicle
      ? vehicleOperationalLabel(item)
      : String(item.freshness || "unavailable").toUpperCase();
    bindPrettyTooltip(
      marker,
...
userAdminRefreshButtons.forEach((button) => {
...
if (adminOrganizationFilter) {
  adminOrganizationFilter.addEventListener("change", () => {
    ...
    setCameraAdminFeedback("");
    void refreshUserAdmin({ preserveDraft: false });
    void refreshVehicleRegistry();
    void refreshVehicleRegistryFormOptions();
  });
}
...
if (telemetryFocusCard) {
  telemetryFocusCard.addEventListener("click", (event) => {
    if (event.target instanceof Element && event.target.closest("#route-toggle-btn")) {
...
if (geofenceDrawName) {
  geofenceDrawName.addEventListener("input", () => {
    setGeofenceDrawFeedback("");
    updateGeofenceDrawControls();
  });
}

if (geofenceDrawColor) {
  geofenceDrawColor.addEventListener("input", () => {
    setGeofenceDrawFeedback("");
    renderGeofenceDraft();
  });
}
...
if (geofenceDrawSave) {
  geofenceDrawSave.addEventListener("click", () => {
    void saveFleetGeofencePolygon();
  });
}

if (geofenceDrawCancel) {
  geofenceDrawCancel.addEventListener("click", () => {
    stopFleetGeofencePolygonDraw();
  });
}
...
document.addEventListener("click", (event) => {
  const editButton = event.target instanceof Element
    ? event.target.closest("[data-geofence-edit]")
    : null;
  if (editButton) {
    void editFleetGeofence(editButton.getAttribute("data-geofence-edit"));
    return;
  }
  const deleteButton = event.target instanceof Element
    ? event.target.closest("[data-geofence-delete]")
    : null;
  if (deleteButton) {
    void deleteFleetGeofence(deleteButton.getAttribute("data-geofence-delete"));
  }
});

document.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && geofenceDrawingActive) {
    stopFleetGeofencePolygonDraw();
  }
});
...
adminTabs.forEach((button) => {
  button.addEventListener("click", () => {
    setActiveAdminTab(button.dataset.adminTab, { updateHash: true });
  });
});
...
window.addEventListener("hashchange", () => {
  setActiveSidebarLink();
  setActiveAdminTab(decodeURIComponent(String(window.location.hash || "").replace(/^#/, "")));
});
...
// ═══════════════════════════════════════════════════════════════════════════
//  Panel de km recorridos - Registro de Dispositivos
// ═══════════════════════════════════════════════════════════════════════════

document.addEventListener("click", (e) => {
  const btn = e.target.closest(".vehicle-km-load");
  if (btn) {
    const panel = btn.closest(".vehicle-km-panel");
    if (panel) loadKmSummary(panel);
  }
});
...
async function loadKmSummary(panel) {
  ...
  try {
    const data = await fetchJson(
      `/api/telemetry/km-summary?vehicle_id=${encodeURIComponent(vehicleId)}&from_date=${from}&to_date=${to}`,
      { timeoutMs: 15000 }
    );
    ...
    resultsEl.innerHTML = `
      ...
      <table class="vehicle-km-table">
        ...
        <tbody>
          ${data.filter((r) => Number(r.km) > 0 || Number(r.points) > 0).map((r) => `
    ...
    const exportHref = `/api/telemetry/km-summary/export?vehicle_id=${encodeURIComponent(vehicleId)}&from_date=${from}&to_date=${to}&label=${encodeURIComponent(label)}`;
    if (exportEl) {
...
// ═══════════════════════════════════════════════════════════════════════════
//  Rutas en el mapa — polyline del recorrido del vehículo seleccionado
// ═══════════════════════════════════════════════════════════════════════════
...
function routePointMarkerOptions() {
  ...
  if (typeof window.L !== "undefined" && typeof window.L.canvas === "function") {
    routePointRenderer = routePointRenderer || window.L.canvas({ padding: 0.25 });
    options.renderer = routePointRenderer;
  }
  return options;
}

function createRoutePointMarker(lat, lon) {
  if (typeof window.L === "undefined") return null;
  return window.L.circleMarker([lat, lon], routePointMarkerOptions());
}

function addRoutePointMarker(lat, lon) {
  if (!mapInstance || typeof window.L === "undefined") return;
  if (!routePointsLayer) {
    routePointsLayer = window.L.layerGroup().addTo(mapInstance);
  }
  const marker = createRoutePointMarker(lat, lon);
  if (marker) {
  ...
  return points
    .map((point) => {
      ...
      return {
        ...
        diagnostics: point?.diagnostics && typeof point.diagnostics === "object" ? point.diagnostics : null,
      };
    })
    .filter(Boolean);
}
...
function classifyLiveRouteSegment(previous, point) {
  if (!previous || !point) return "start";
  const distanceKm = routeDistanceKm(previous, point);
  const prevTs = Date.parse(previous.timestamp || "");
  const nextTs = Date.parse(point.timestamp || "");
  const elapsedSeconds = Number.isFinite(prevTs) && Number.isFinite(nextTs)
    ? (nextTs - prevTs) / 1000
    : 0;
  const impliedSpeed = elapsedSeconds > 0 ? distanceKm / (elapsedSeconds / 3600) : 0;
  point.distance_km = distanceKm;
  point.elapsed_seconds = elapsedSeconds;
  point.implied_speed_kmh = impliedSpeed;
  if (elapsedSeconds <= 0 && previous.timestamp && point.timestamp) return "suspicious";
  if (elapsedSeconds > ROUTE_SEGMENT_MAX_GAP_SECONDS || distanceKm > ROUTE_SEGMENT_MAX_DISTANCE_KM) return "gap";
  if (impliedSpeed > ROUTE_SEGMENT_MAX_SPEED_KMH) return "suspicious";
  return "normal";
}

function addRouteSegment(previous, point) {
...
function clearRoute() {
  ...
  if (routePointsLayer && mapInstance) {
    mapInstance.removeLayer(routePointsLayer);
    routePointsLayer = null;
  }
  if (routeStartMarker && mapInstance) {
    mapInstance.removeLayer(routeStartMarker);
    routeStartMarker = null;
  }
  if (routeEndMarker && mapInstance) {
    mapInstance.removeLayer(routeEndMarker);
    routeEndMarker = null;
  }
  routeDeviceId = null;
  routeDate = null;
...
async function loadVehicleRoute(vehicleSourceId, dateStr) {
  ...
  try {
    ...
    routePolyline = window.L.layerGroup().addTo(mapInstance);
    addRouteSegments(routePoints);

    const pointMarkers = routePoints
      .map((point) => createRoutePointMarker(point.lat, point.lon))
      .filter(Boolean);
    routePointsLayer = window.L.layerGroup(pointMarkers).addTo(mapInstance);
    ...
    if (!telemetryMapManualControl) {
      markTelemetryMapProgrammaticInteraction();
      fitMapToCoordinates(mapInstance, latlngs, { maxZoom: 15, singleZoom: 15 });
    }
  } catch (err) {}
  _updateRouteToggleBtn();
}
...
function syncVehicleRoute(items) {
  ...
  if (isNewVehicle || isStale) {
    ...
    const timestamp = String(selected.extra?.gps_datetime_iso || selected.extra?.gps_datetime || selected.received_at || selected.timestamp || "");
    const isDuplicatePoint = _lastRoutePoint
      && lat === _lastRoutePoint.lat
      && lon === _lastRoutePoint.lon
      && (!timestamp || !_lastRoutePoint.timestamp || timestamp === _lastRoutePoint.timestamp);
    if (
      Number.isFinite(lat) && Number.isFinite(lon) &&
      !isDuplicatePoint
    ) {
      const nextPoint = { lat, lon, timestamp, speed: toFiniteNumber(selected.speed), heading: toFiniteNumber(selected.heading) };
      nextPoint.segment_status = classifyLiveRouteSegment(_lastRoutePoint, nextPoint);
      addRouteSegment(_lastRoutePoint, nextPoint);
      addRoutePointMarker(lat, lon);
      _lastRoutePoint = nextPoint;
      if (routeEndMarker) routeEndMarker.setLatLng([lat, lon]);
    }
  }
  _updateRouteToggleBtn();
}
...
function initReportsPage() {
  ...
  const cameraSelect = qs("#reports-camera-id");
  const gapInput = qs("#reports-gap-minutes");
  const refreshBtn = qs("#reports-refresh");

  const dailyFrom = qs("#reports-daily-from");
  const dailyTo = qs("#reports-daily-to");
  const dailyLoad = qs("#reports-daily-load");
  const dailyExport = qs("#reports-daily-export");
  const dailyBody = qs("#reports-daily-body");
  const dailyEmpty = qs("#reports-daily-empty");
  const dailyCount = qs("#reports-daily-count");

  const personIdInput = qs("#reports-person-id");
  const individualFrom = qs("#reports-individual-from");
  const individualTo = qs("#reports-individual-to");
  const individualDay = qs("#reports-individual-day");
  const individualLoad = qs("#reports-individual-load");
  const individualExport = qs("#reports-individual-export");
  const individualBody = qs("#reports-individual-body");
  const individualEmpty = qs("#reports-individual-empty");
  const sessionsBody = qs("#reports-sessions-body");
  const sessionsEmpty = qs("#reports-sessions-empty");

  const monthInput = qs("#reports-month");
  const monthLoad = qs("#reports-month-load");
  const monthExport = qs("#reports-month-export");
  const monthBody = qs("#reports-month-body");
  const monthEmpty = qs("#reports-month-empty");
  const monthCount = qs("#reports-month-count");

  const platesFrom = qs("#reports-plates-from");
  const platesTo = qs("#reports-plates-to");
  const platesLoad = qs("#reports-plates-load");
  const platesExport = qs("#reports-plates-export");
  const platesBody = qs("#reports-plates-body");
  const platesEmpty = qs("#reports-plates-empty");
  const platesCount = qs("#reports-plates-count");

  const kpiPeople = qs("#reports-kpi-people");
  const kpiPersonEvents = qs("#reports-kpi-person-events");
  const kpiPlates = qs("#reports-kpi-plates");
  const kpiCameras = qs("#reports-kpi-cameras");
  ...
  function setDefaults() {
    const today = ecDate();
    const weekAgo = ecDate(-6);
    if (dailyFrom && !dailyFrom.value) dailyFrom.value = today;
    [individualFrom, platesFrom].forEach((input) => {
      if (input && !input.value) input.value = weekAgo;
    });
    [dailyTo, individualTo, platesTo].forEach((input) => {
  ...
  function fmtInt(value) {
    return Math.round(numeric(value, 0)).toLocaleString("es-EC");
  }

  function fmtHours(value) {
    const parsed = numeric(value, 0);
    return parsed ? parsed.toFixed(2) : "0.00";
  }
  ...
  function cameraParam() {
    return String(cameraSelect?.value || "").trim();
  }

  function gapParam() {
    const value = Math.round(numeric(gapInput?.value, 15));
    return String(Math.max(1, Math.min(value, 720)));
  }
  ...
  function makeParams(values) {
    ...
    const cameraId = cameraParam();
    if (cameraId) params.set("camera_id", cameraId);
    return params;
  }

  function setExportHref(anchor, path, params) {
    if (!anchor) return;
    const query = params ? params.toString() : "";
    if (!query || query.includes("person_id=&")) {
      anchor.href = "#";
      anchor.setAttribute("aria-disabled", "true");
      return;
    }
    anchor.href = `/api/reports/${path}?${query}`;
    anchor.removeAttribute("aria-disabled");
  }

  function setLoading(button, loading, label = "Cargando") {
    if (!button) return;
    if (loading) {
  ...
  async function fetchReport(path, params, timeoutMs = 60000) {
    const query = params ? params.toString() : "";
    return await fetchJson(`/api/reports/${path}${query ? `?${query}` : ""}`, { timeoutMs });
  }
  ...
  async function populateCameras() {
    ...
    CAMERAS.forEach((camera) => {
      const id = String(camera.codigo_unico || camera.unique_code || camera.name || camera.camera_id || "").trim();
      if (!id) return;
      const label = String(camera.display_name || camera.nombre || camera.name || camera.camera_name || id).trim();
      options.set(id, label || id);
    });
    try {
      const rows = await fetchReport("cameras", null, 15000);
      (Array.isArray(rows) ? rows : []).forEach((row) => {
        const id = String(row.camera_id || "").trim();
        if (!id) return;
        options.set(id, String(row.camera_name || id).trim() || id);
      });
    } catch (error) {
      console.warn("[reports] No se pudieron cargar cámaras:", error);
    }
    const selected = cameraSelect.value;
    cameraSelect.innerHTML = '<option value="">Todas</option>' + Array.from(options.entries())
      .sort((a, b) => a[1].localeCompare(b[1], "es"))
      .map(([id, label]) => `<option value="${escapeHtml(id)}">${escapeHtml(label)}</option>`)
      .join("");
    if (selected && options.has(selected)) cameraSelect.value = selected;
  }

  async function loadOverview() {
    const from = dailyFrom?.value || platesFrom?.value || ecDate(-6);
    const to = dailyTo?.value || platesTo?.value || ecDate();
    try {
      const data = await fetchReport("overview", makeParams({ from_date: from, to_date: to }), 30000);
      if (kpiPeople) kpiPeople.textContent = fmtInt(data.people_detected);
      if (kpiPersonEvents) kpiPersonEvents.textContent = fmtInt(data.person_events);
      if (kpiPlates) kpiPlates.textContent = fmtInt(data.plates_detected);
      if (kpiCameras) kpiCameras.textContent = fmtInt(data.active_cameras);
    } catch (error) {
      [kpiPeople, kpiPersonEvents, kpiPlates, kpiCameras].forEach((el) => {
  ...
  function syncExports() {
    setExportHref(dailyExport, "personnel/daily/export", makeParams({
      from_date: dailyFrom?.value,
      to_date: dailyTo?.value,
      gap_minutes: gapParam(),
    }));
    const [year, month] = String(monthInput?.value || "").split("-");
    setExportHref(monthExport, "personnel/monthly/export", makeParams({
      year,
      month,
      gap_minutes: gapParam(),
    }));
    setExportHref(platesExport, "plates/export", makeParams({
      from_date: platesFrom?.value,
      to_date: platesTo?.value,
    }));
    const individualPersonId = String(personIdInput?.value || "").trim();
    setExportHref(individualExport, "personnel/sessions/export", makeParams({
      person_id: individualPersonId,
      from_date: individualFrom?.value,
      to_date: individualTo?.value,
      work_date: individualDay?.value,
      gap_minutes: gapParam(),
    }));
    if (individualExport && !individualPersonId) {
  ...
  async function loadDaily() {
    if (!dailyFrom?.value || !dailyTo?.value) return;
    setLoading(dailyLoad, true);
    syncExports();
    try {
      const rows = await fetchReport("personnel/daily", makeParams({
        from_date: dailyFrom.value,
        to_date: dailyTo.value,
        gap_minutes: gapParam(),
      }));
      state.dailyLoaded = true;
      if (dailyCount) dailyCount.textContent = `${fmtInt(rows.length)} registros`;
      renderRows(dailyBody, dailyEmpty, rows, (row) => `
        <tr data-report-person-id="${escapeHtml(row.person_id || "")}">
          <td><strong>${escapeHtml(row.person_id || "--")}</strong></td>
          <td>${escapeHtml(row.person_name || "--")}</td>
          <td>${escapeHtml(row.work_date || "--")}</td>
          <td>${escapeHtml(fmtTime(row.first_seen))}</td>
          <td>${escapeHtml(fmtTime(row.last_seen))}</td>
          <td>${escapeHtml(fmtHours(row.hours))}</td>
          <td>${fmtInt(row.reentries)}</td>
          <td>${fmtInt(row.detections)}</td>
          <td>${escapeHtml(row.cameras || "--")}</td>
        </tr>
      `, 9);
      await loadOverview();
    } catch (error) {
      if (dailyCount) dailyCount.textContent = "Error";
      renderError(dailyBody, dailyEmpty, error.message, 9);
    } finally {
      setLoading(dailyLoad, false);
    }
  }

  function fillIndividualDays(rows) {
    if (!individualDay) return;
    const current = individualDay.value;
    const dates = Array.from(new Set((Array.isArray(rows) ? rows : []).map((row) => String(row.work_date || "").trim()).filter(Boolean)));
    individualDay.innerHTML = '<option value="">Todos</option>' + dates
      .map((day) => `<option value="${escapeHtml(day)}">${escapeHtml(day)}</option>`)
      .join("");
    if (current && dates.includes(current)) individualDay.value = current;
  }

  async function loadSessions() {
    const personId = String(personIdInput?.value || "").trim();
    if (!personId || !individualFrom?.value || !individualTo?.value) return;
    syncExports();
    try {
      const rows = await fetchReport("personnel/sessions", makeParams({
        person_id: personId,
        from_date: individualFrom.value,
        to_date: individualTo.value,
        work_date: individualDay?.value,
      ...
      renderRows(sessionsBody, sessionsEmpty, rows, (row) => `
        <tr>
          <td>${escapeHtml(row.work_date || "--")}</td>
          <td><strong>${fmtInt(row.session_no)}</strong></td>
          <td>${escapeHtml(fmtTime(row.entry_at))}</td>
          <td>${escapeHtml(fmtTime(row.exit_at))}</td>
          <td>${escapeHtml(fmtMinutes(row.minutes_since_previous_exit))}</td>
          <td>${fmtInt(row.detections)}</td>
          <td>${escapeHtml(row.cameras || "--")}</td>
        </tr>
      `, 7);
    } catch (error) {
      renderError(sessionsBody, sessionsEmpty, error.message, 7);
    }
  }
  ...
  async function loadIndividual() {
    const personId = String(personIdInput?.value || "").trim();
    if (!personId || !individualFrom?.value || !individualTo?.value) return;
    setLoading(individualLoad, true);
    syncExports();
    try {
      const rows = await fetchReport("personnel/individual", makeParams({
        person_id: personId,
        from_date: individualFrom.value,
        to_date: individualTo.value,
        gap_minutes: gapParam(),
      }));
      state.individualLoaded = true;
      fillIndividualDays(rows);
      renderRows(individualBody, individualEmpty, rows, (row) => `
        <tr data-report-work-date="${escapeHtml(row.work_date || "")}">
      ...
      renderError(individualBody, individualEmpty, error.message, 7);
      renderRows(sessionsBody, sessionsEmpty, [], () => "", 7);
    } finally {
      setLoading(individualLoad, false);
    }
  }
  ...
  async function loadMonthly() {
    ...
    setLoading(monthLoad, true);
    syncExports();
    try {
      const rows = await fetchReport("personnel/monthly", makeParams({
        year,
        month,
        gap_minutes: gapParam(),
      }));
      state.monthLoaded = true;
      if (monthCount) monthCount.textContent = `${fmtInt(rows.length)} personas`;
      renderRows(monthBody, monthEmpty, rows, (row) => `
        <tr data-report-person-id="${escapeHtml(row.person_id || "")}">
          <td><strong>${escapeHtml(row.person_id || "--")}</strong></td>
          <td>${escapeHtml(row.person_name || "--")}</td>
          <td>${fmtInt(row.days_present)}</td>
          <td>${escapeHtml(fmtHours(row.total_hours))}</td>
          <td>${escapeHtml(fmtHours(row.avg_hours_day))}</td>
          <td>${fmtInt(row.sessions)}</td>
          <td>${fmtInt(row.reentries)}</td>
          <td>${fmtInt(row.detections)}</td>
        </tr>
      ...
      if (monthCount) monthCount.textContent = "Error";
      renderError(monthBody, monthEmpty, error.message, 8);
    } finally {
      setLoading(monthLoad, false);
    }
  }

  async function loadPlates() {
    if (!platesFrom?.value || !platesTo?.value) return;
    setLoading(platesLoad, true);
    syncExports();
    try {
      const rows = await fetchReport("plates/stats", makeParams({
        from_date: platesFrom.value,
        to_date: platesTo.value,
      }));
      state.platesLoaded = true;
      if (platesCount) platesCount.textContent = `${fmtInt(rows.length)} placas`;
      renderRows(platesBody, platesEmpty, rows, (row) => `
        <tr>
          <td><strong class="reports-plate">${escapeHtml(row.plate || "--")}</strong></td>
          <td>${escapeHtml(row.camera_name || "--")}</td>
          <td>${fmtInt(row.detections)}</td>
          <td>${fmtInt(row.days_detected)}</td>
          <td>${escapeHtml(fmtDateTime(row.first_seen))}</td>
          <td>${escapeHtml(fmtDateTime(row.last_seen))}</td>
        </tr>
      `, 6);
      await loadOverview();
    } catch (error) {
      if (platesCount) platesCount.textContent = "Error";
      renderError(platesBody, platesEmpty, error.message, 6);
    } finally {
      setLoading(platesLoad, false);
    }
  }

  function activeReportsTab() {
    return String(qs(".reports-tab.is-active")?.dataset.reportsTab || "personnel");
  }

  function activePersonnelTab() {
    return String(qs(".reports-subtab.is-active")?.dataset.reportsPersonnelTab || "daily");
  }

  async function refreshActive() {
    syncExports();
    if (activeReportsTab() === "plates") {
      await loadPlates();
      return;
    }
    const tab = activePersonnelTab();
    if (tab === "individual") await loadIndividual();
    else if (tab === "monthly") await loadMonthly();
    else await loadDaily();
  }

  function activateMainTab(name) {
    qsa("[data-reports-tab]").forEach((button) => {
      button.classList.toggle("is-active", button.dataset.reportsTab === name);
    });
    qsa("[data-reports-panel]").forEach((panel) => {
      panel.classList.toggle("is-active", panel.dataset.reportsPanel === name);
    });
    if (name === "plates" && !state.platesLoaded) void loadPlates();
  }

  function activatePersonnelTab(name) {
    qsa("[data-reports-personnel-tab]").forEach((button) => {
      button.classList.toggle("is-active", button.dataset.reportsPersonnelTab === name);
    });
    qsa("[data-reports-personnel-panel]").forEach((panel) => {
      panel.classList.toggle("is-active", panel.dataset.reportsPersonnelPanel === name);
    });
    if (name === "daily" && !state.dailyLoaded) void loadDaily();
    if (name === "monthly" && !state.monthLoaded) void loadMonthly();
    if (name === "individual" && !state.individualLoaded && String(personIdInput?.value || "").trim()) void loadIndividual();
  }

  qsa("[data-reports-tab]").forEach((button) => {
    button.addEventListener("click", () => activateMainTab(String(button.dataset.reportsTab || "personnel")));
  });
  qsa("[data-reports-personnel-tab]").forEach((button) => {
    button.addEventListener("click", () => activatePersonnelTab(String(button.dataset.reportsPersonnelTab || "daily")));
  });

  dailyLoad?.addEventListener("click", () => void loadDaily());
  individualLoad?.addEventListener("click", () => void loadIndividual());
  monthLoad?.addEventListener("click", () => void loadMonthly());
  platesLoad?.addEventListener("click", () => void loadPlates());
  refreshBtn?.addEventListener("click", () => void refreshActive());
  cameraSelect?.addEventListener("change", () => void refreshActive());
  gapInput?.addEventListener("change", () => void refreshActive());
  individualDay?.addEventListener("change", () => void loadSessions());

  [dailyFrom, dailyTo, individualFrom, individualTo, platesFrom, platesTo, monthInput].forEach((input) => {
    input?.addEventListener("change", syncExports);
  });

  personIdInput?.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
      void loadIndividual();
    }
  ...
  dailyBody?.addEventListener("click", (event) => {
    const row = event.target instanceof Element ? event.target.closest("[data-report-person-id]") : null;
    const personId = String(row?.dataset.reportPersonId || "").trim();
    if (!personId || !personIdInput) return;
    personIdInput.value = personId;
    if (individualFrom && dailyFrom?.value) individualFrom.value = dailyFrom.value;
    if (individualTo && dailyTo?.value) individualTo.value = dailyTo.value;
    activateMainTab("personnel");
    activatePersonnelTab("individual");
    void loadIndividual();
  });

  individualBody?.addEventListener("click", (event) => {
    const row = event.target instanceof Element ? event.target.closest("[data-report-work-date]") : null;
    const workDate = String(row?.dataset.reportWorkDate || "").trim();
    if (!workDate || !individualDay) return;
    individualDay.value = workDate;
    syncExports();
    void loadSessions();
  });
  ...
  setDefaults();
  syncExports();
  void populateCameras().then(() => Promise.all([loadOverview(), loadDaily()]));
}

initReportsPage();

// ═══════════════════════════════════════════════════════════════════════════
//  Modal de kilómetros de FLOTA — Registro de Dispositivos
// ═══════════════════════════════════════════════════════════════════════════

{
  const fleetModal   = document.getElementById("fleet-km-modal");
  const fleetOpenBtn = document.getElementById("fleet-km-open");
  const fleetCloseBtn= document.getElementById("fleet-km-close");
  const fleetBackdrop= document.getElementById("fleet-km-backdrop");
  const fleetFromEl  = document.getElementById("fleet-km-from");
  const fleetToEl    = document.getElementById("fleet-km-to");
  const fleetLoadBtn = document.getElementById("fleet-km-load");
  const fleetResultsEl = document.getElementById("fleet-km-results");
  ...
  const _ecToday = () => new Date(Date.now() - 5 * 3600 * 1000).toISOString().slice(0, 10);
  const _ecMinus = (days) => new Date(Date.now() - 5 * 3600 * 1000 - days * 86400000).toISOString().slice(0, 10);
  const _ecMonthStart = () => {
    const now = new Date(Date.now() - 5 * 3600 * 1000);
    return `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, "0")}-01`;
  };

  function setFleetRange(mode, { load = false } = {}) {
    if (!fleetFromEl || !fleetToEl) return;
    const today = _ecToday();
    const normalizedMode = String(mode || "week").trim().toLowerCase();
    if (normalizedMode === "today") {
      fleetFromEl.value = today;
      fleetToEl.value = today;
    } else if (normalizedMode === "month") {
      fleetFromEl.value = _ecMonthStart();
      fleetToEl.value = today;
    } else {
      fleetFromEl.value = _ecMinus(6);
      fleetToEl.value = today;
    }
    fleetFromEl.max = today;
    fleetToEl.max = today;
    if (load && fleetLoadBtn) {
      fleetLoadBtn.click();
    }
  }

  function openFleetModal() {
    if (!fleetFromEl?.value || !fleetToEl?.value) {
      setFleetRange("week");
    } else {
      const today = _ecToday();
      fleetFromEl.max = today;
  ...
  if (fleetBackdrop)  fleetBackdrop.addEventListener("click", closeFleetModal);
  document.addEventListener("keydown", (e) => { if (e.key === "Escape" && !fleetModal.hidden) closeFleetModal(); });

  let _fleetReport = null;

  if (fleetLoadBtn && fleetResultsEl) {
    fleetLoadBtn.addEventListener("click", async () => {
      const from = fleetFromEl?.value;
      const to   = fleetToEl?.value;
      if (!from || !to) return;
      fleetResultsEl.innerHTML = '<div class="vehicle-km-loading">Calculando reporte agregado de flota...</div>';
      if (fleetExportBtn) fleetExportBtn.style.display = "none";
      fleetLoadBtn.disabled = true;
      try {
        const params = new URLSearchParams({
          from_date: from,
          to_date: to,
        ...
        const payload = await fetchJson(
          `/api/telemetry/km-fleet?${params.toString()}`,
          { timeoutMs: 45000 }
        );
        const report = normalizeFleetReport(payload, from, to);
        if (!report.vehicles.length && !report.daily.some((row) => Number(row.total_points || 0) > 0)) {
          fleetResultsEl.innerHTML = '<div class="vehicle-km-empty">Sin datos para el período seleccionado.</div>';
          return;
        }
        _fleetReport = report;
        renderFleetReport(report, from, to);
        if (fleetExportBtn) fleetExportBtn.style.display = "inline-block";
      } catch (err) {
        fleetResultsEl.innerHTML = `<div class="vehicle-km-error">Error: ${escapeHtml(String(err?.message || err))}</div>`;
      } finally {
        fleetLoadBtn.disabled = false;
      }
    });
  }

  if (fleetModeEl) {
    fleetModeEl.addEventListener("change", () => {
      if (!_fleetReport) return;
      renderFleetReport(_fleetReport, fleetFromEl?.value || "", fleetToEl?.value || "");
    });
  }
  ...
  function normalizeFleetReport(payload, from, to) {
    if (Array.isArray(payload)) {
      ...
      const totalKm = vehicles.reduce((sum, row) => sum + Number(row.total_km || 0), 0);
      const totalPoints = vehicles.reduce((sum, row) => sum + Number(row.total_points || 0), 0);
      return {
        from_date: from,
        to_date: to,
        range_days: Math.round((new Date(to) - new Date(from)) / 86400000) + 1,
        totals: {
          total_km: totalKm,
          active_vehicles: vehicles.filter((row) => Number(row.total_km || 0) > 0.05).length,
    ...
    return {
      from_date: report.from_date || from,
      to_date: report.to_date || to,
      range_days: Number(report.range_days || (Math.round((new Date(to) - new Date(from)) / 86400000) + 1) || 0),
      totals: {
        total_km: Number(totals.total_km || vehicles.reduce((sum, row) => sum + Number(row.total_km || 0), 0)),
        active_vehicles: Number(totals.active_vehicles || vehicles.filter((row) => Number(row.total_km || 0) > 0.05).length),
        total_vehicles: Number(totals.total_vehicles || vehicles.length),
        total_points: Number(totals.total_points || vehicles.reduce((sum, row) => sum + Number(row.total_points || 0), 0)),
        active_days: Number(totals.active_days || daily.filter((row) => Number(row.total_km || 0) > 0.05).length),
        max_speed: Number(totals.max_speed || 0),
      },
      vehicles,
      daily,
      monthly,
